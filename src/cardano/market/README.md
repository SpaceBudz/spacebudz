...
This module contains the full logic of the SpaceBudz marketplace.
The `contract.hs` file contains the validator and the off-chain code written in Haskell. For the actual marketplace on the website the off-chain code was rewritten in JavaScript (`./index.js`) in combination with a customized version of the [serialization-lib](https://github.com/Emurgo/cardano-serialization-lib). This customized version you can find under `./custom_modules` (for browser and NodeJs).
...
**Note**
With the default setup it is expected that the marketplace is run the browser and in combination with [Nami Wallet](https://namiwallet.io), because it is tightly integrated into the marketplace. Also we recommend to use Webpack 5 and set `syncWebAssambly` to `true` (or `asyncWebAssambly` in case the latter doesn't work for you) in the `webpack.config.js` file.
